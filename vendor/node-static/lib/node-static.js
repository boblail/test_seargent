...
this.Server = function(root, options) {
  ...
  this.serveDir = function (pathname, req, res, finish) {
      var htmlIndex = path.join(pathname, 'index.html'),
          that = this;

      fs.stat(htmlIndex, function (e, stat) {
          if (!e) {
              self.respond(null, 200, {}, [htmlIndex], stat, req, res, finish);
          } else {
      ...
      function streamFiles(files) {
          util.mstat(pathname, files, function (e, stat) {
              self.respond(pathname, 200, {}, files, stat, req, res, finish);
          });
      }
  };
  this.serveFile = function (pathname, status, headers, req, res) {
      var that = this;
  ...
  this.finish = function (status, headers, req, res, promise, callback) {
      var result = {
          status:  status,
          headers: headers,
  ...
  this.servePath = function (pathname, status, headers, req, res, finish) {
      var that = this,
      ...
      if (new(RegExp)('^' + self.root).test(pathname)) {
          fs.stat(pathname, function (e, stat) {
              if (e) {
                  finish(404, {});
                  ...
                  self.serveDir(pathname, req, res, finish);
              } else {
                  finish(400, {});
              }
          });
  ...
  this.normalize = function (pathname) {
      return path.normalize(path.join(this.root, pathname));
  };
  this.serve = function (req, res, callback) {
      var that = this,
          promise = new(events.EventEmitter);
  ...
  this.respond = function (pathname, status, _headers, files, stat, req, res, finish) {
  ...
  this.stream = function (pathname, files, buffer, res, callback) {
